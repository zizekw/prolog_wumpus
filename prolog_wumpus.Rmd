...
Here we can `update_time` if we take the `time_taken` and update a `NewTime` by coercing an addition function. Then `retractall` removes existing facts within `time_taken` before we then update with the `NewTime`.  
